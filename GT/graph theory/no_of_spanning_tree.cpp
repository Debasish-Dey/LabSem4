...
double determinant(vector<vector<double>> &mat){
    ...
    for (int i = 0; i < n; i++){
        int maxRow = i;
        for (int j = i + 1; j < n; j++){
            if (fabs(mat[j][i]) > fabs(mat[maxRow][i])) maxRow = j;
        }
        if (maxRow != i){
...
int main(){
    ...
    if (isComplete(graph, degree)){
        cout << "Given graph is a complete graph\n";
        cout << "Number of spanning trees of this graph: " << pow(v1, v1 - 2) << endl;
    }
    else{
        ...
        for (int i = 1; i < v1; i++){
            for (int j = 1; j < v1; j++) reducedMatrix[i - 1][j - 1] = laplacian[i][j];
        }
        cout << "Number of spanning trees of this graph: " << abs(determinant(reducedMatrix)) << endl;
    }
}